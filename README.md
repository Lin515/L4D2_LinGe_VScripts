...
- 伤害统计
  - 对特感伤害统计  
    真人玩家对非 Witch 与 Tank 以外的特感的伤害将会被累计记录。
    - !hd 立即输出当前的特感伤害统计。特感伤害统计还包括了友伤数据。
    - !hd auto n 设置每间隔 n 秒自动输出一次特感伤害统计。  
      若 n=0，则只在回合结束时自动输出；若 n<0，则彻底关闭自动输出。
    - !hd player n 设置特感伤害统计最多显示多少人。该数据同时影响 Tank 的伤害统计，若 n<=0，则会彻底关闭特感、Tank 伤害统计的显示。

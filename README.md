...
- 伤害与击杀统计  
  真人玩家对非 Witch 与 Tank 以外的特感的伤害将会被累计记录，同时会被记录各自的特感与小丧尸击杀数。  
  伤害与击杀统计分为 HUD 排行榜与聊天窗排行榜，可以在配置文件中自定义两个排行榜的显示内容，具体请查看[配置文件](#HUD)。
  - HUD 排行  
    可以显示 1~8 人，默认显示前3名。  
  ...
  - 聊天窗排行  
    在聊天窗输出排行榜数据。
    - !hd 立即输出聊天窗排行榜。
    - !hd auto n 设置每间隔 n 秒自动输出一次排行榜。  
      若 n=0，则只在回合结束时自动输出；若 n<0，则彻底关闭自动输出。
    - !hd player n 设置聊天窗排行榜最多显示多少人。该数据同时影响 Tank 的伤害统计，若 n<=0，则会彻底关闭聊天窗排行榜、Tank 伤害统计的显示。
  - 对TANK伤害统计  
    真人玩家对 Tank 的伤害将被临时记录，在所有 Tank 死亡后数据将被输出显示。
  - 友伤统计  
    友伤数据会被记录并且随特感伤害统计一起输出显示。  
    此外还有即时性的友伤提示信息，使用 !thi n 指令可以改变友伤提示的形式。  
    - !thi 0 关闭友伤提示。
    - !thi 1 友伤提示：所有人可见，公开处刑式。
    - !thi 2 友伤提示：仅双方可见。
- 玩家状态提示  
	在玩家遇到倒地、死亡、黑白等情况时，会自动触发聊天窗提示。（BOT不触发）  
	!playerstate n 可设置提示风格，n=0则关闭，n=1为服务器式提示，n=2为玩家自言自语呼救式提示
...
- hurt  
  ...
  - versusNoHUDRank 对抗模式下是否不显示排行榜。
  - HUDRank 排行榜最多显示多少人，范围 0~8。

  - teamHurtInfo 友伤提示，0 关闭，1 公开处刑式，2 仅双方可见。
  - autoPrint 自动输出特感伤害统计的间隔，单位为秒。为 0 则只在回合结束时输出，<0 则永不自动输出。
  - hurtRank 特感、Tank 伤害统计最多显示多少人的数据，若 <= 0 则会关闭伤害统计的显示。
- style  
  玩家数量的显示风格，0 自动，1 普通风格（活跃：x 摸鱼：x 空位：x），2 对抗风格（生还：x vs 特感：x）
- isShowTime  
  是否显示时间。
- textHeight  
  HUD 中一行文字的高度，百分比数。

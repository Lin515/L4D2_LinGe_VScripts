...
请下载右侧的 Release 发布文件中的 LinGe_VScripts.zip 文件，将其解压到 left4dead2 目录下。其中除了打包好的脚本 MOD 文件外，还包含了辅助插件以及 sourcemod 插件平台。如果需要使用插件请在游戏启动项中加入 -insecure。

本来打算把辅助插件用 C++/L4D2SDK 重写一下，这样可以脱离 sourcemod 平台运行。不过最近很少打求生了，以后如果有兴趣了就再写一下，暂时搁置，现在就配合插件平台将就用吧。

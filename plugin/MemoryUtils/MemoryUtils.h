...
// 从虚函数表获取指定的虚函数
inline std::uintptr_t GetVirtualFunction(void *base, VTableIndex index)
{
	// 将 base 对象基址直接转换为指向虚函数表的指针
	std::uintptr_t **pVTable = (std::uintptr_t **)base;
	// 从虚函数表获取对应的虚函数指针
#ifdef WIN32
	return (*pVTable)[index.Windows];
#elif defined _LINUX
	return (*pVTable)[index.Linux];
#endif
}
...
public:
	...
	inline void *FindSignature(Signature signature)
	{
#ifdef WIN32
		return FindPattern(signature.Windows);
#elif defined PLATFORM_LINUX
		return ResolveSymbol(signature.Linux);
#endif
	}
